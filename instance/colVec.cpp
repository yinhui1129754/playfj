...
float colVec::pointAngleInfo(const utils::usePoint point1, const utils::usePoint point2) {
	...
	float angle = floor(180 / (M_PI / radina));//将弧度转换成角度
...
float colVec::pointAngleInfo(const POINT point1, const POINT point2) {
	...
	if (point2.x > point1.x&&point2.y > point1.y) {//第一象限
		return atan((float)(point2.y - point1.y) / (point2.x - point1.x)) / M_PI * 180.0f;
	}
	else if (point2.x<point1.x&&point2.y>point1.y) {
		return atan((float)(point1.x - point2.x) / (point2.y - point1.y)) / M_PI * 180.0f + 90.0f;
	}
	else if (point2.x < point1.x&&point2.y < point1.y) {
		return atan((float)(point1.y - point2.y) / (point1.x - point2.x)) / M_PI * 180.0f + 180.0f;
	}
	else if (point2.x > point1.x&&point2.y < point1.y) {
		return atan((float)(point2.x - point1.x) / (point1.y - point2.y)) / M_PI * 180.0f + 270.0f;
	}
	if (point2.x == point1.x&&point2.y > point1.y) {
		return 90.0f;
	}
