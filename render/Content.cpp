...
void Content::resize(UINT w, UINT h) {
	this->m_render->Resize(D2D1::SizeU(w, h));
}
...
void Content::setFontSize(float size) {
...
IDWriteTextFormat * Content::createFont(float size, LPWSTR fontfamliy) {
	IDWriteTextFormat * u_forMat = NULL;
	this->m_writeFac->CreateTextFormat(
		L"微软雅黑",
		NULL,
		DWRITE_FONT_WEIGHT_NORMAL,
...
void Content::setAntialiasMode(D2D1_ANTIALIAS_MODE a) {
	this->m_render->SetAntialiasMode(a);
}
...
void Content::drawText(LPWSTR  txt, int x, int y, IDWriteTextFormat * u_forMat) {
	...
	this->m_render->DrawText(txt, wcslen(txt), u_forMat, layoutRect, this->pen);
}
